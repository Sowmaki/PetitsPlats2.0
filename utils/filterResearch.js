...
Object.entries(filtersObject).forEach(([key, filter]) => {
  ...
  async function updateSuggestionsList() {
    ...
    const filteredIngredients = filteredRecipes.flatMap(recipe => { recipe.ingredients.map(ingredient => ingredient.ingredient.toLowerCase()) })
    const filteredAppliances = filteredRecipes.flatMap(recipe => { recipe.appliance.toLowerCase() })
    const filteredUstensils = filteredRecipes.flatMap(recipe => { recipe.ustensils.map(ustensil => ustensil.toLowerCase()) })


    // Affiche uniquement les suggestions pertinentes
